document.addEventListener("DOMContentLoaded",async()=>{const cfg=window.YUBA_CONFIG,$=id=>document.getElementById(id),configured=cfg.SUPABASE_URL.startsWith("https://")&&!cfg.SUPABASE_ANON_KEY.startsWith("COLE_");if(!configured){$("login-status").textContent="Configure js/config.js antes de usar o painel.";return}const db=supabase.createClient(cfg.SUPABASE_URL,cfg.SUPABASE_ANON_KEY);let rows=[];
async function session(){const{data}=await db.auth.getSession();toggle(!!data.session);if(data.session)load()}
function toggle(on){$("login-card").classList.toggle("oculto",on);$("admin-conteudo").classList.toggle("oculto",!on)}
$("entrar").onclick=async()=>{const{error}=await db.auth.signInWithPassword({email:$("email").value.trim(),password:$("senha").value});$("login-status").textContent=error?error.message:"Login realizado.";if(!error){toggle(true);load()}};
$("sair").onclick=async()=>{await db.auth.signOut();toggle(false)};
async function load(){const{data,error}=await db.from("delivery_areas").select("*").order("price",{ascending:true,nullsFirst:false});if(error){$("admin-status").textContent=error.message;return}rows=data;render()}
